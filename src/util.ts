...
import { Ref, Lerna, Pkg, Result } from './types.js'
...
export function yarnlize(
  dir: string,
  ref: Ref,
  pkg?: Pkg,
): Result<'yarn' | 'bolt'> | undefined {
  if (pkg) {
    if (ref.pkg == null) {
      ref.pkg = dir
    }
...
export function lernalize(
  dir: string,
  lerna?: Lerna,
): Result<'lerna'> | undefined {
...
export function pnpmlize(
  dir: string,
  exists: boolean,
): Result<'pnpm'> | undefined {
  if (exists) {
...
export function rollup(result: Result | undefined, ref: Ref) {
  if (ref.pkg == null) {
    throw new Error(
      `No package.json could be found upwards from the directory ${ref.cwd}`,
    )
  }
